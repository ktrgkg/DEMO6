...
class JobDetailController
  ...
  Future<String?> publishJob() async {
    final current = state.valueOrNull;
    if (current == null) {
      return "Không thể tải thông tin công việc.";
    }
    ...
    try {
      final repository = ref.read(jobRepositoryProvider);
      final job = await repository.publishJob(_jobId);
      state = AsyncData(current.copyWith(job: job, isSubmitting: false));
  ...
  Future<String?> cancelPosterJob() async {
    final current = state.valueOrNull;
    if (current == null) {
      return "Không thể tải thông tin công việc.";
    }
    ...
    try {
      final repository = ref.read(jobRepositoryProvider);
      final job = await repository.cancelPoster(_jobId);
      state = AsyncData(current.copyWith(job: job, isSubmitting: false));
      ref.invalidate(posterJobsControllerProvider);
      return null;
    } on DioException catch (error) {
  ...
  Future<String?> confirmComplete() async {
    final current = state.valueOrNull;
    if (current == null) {
      return "Không thể tải thông tin công việc.";
    }
    ...
    try {
      final repository = ref.read(jobRepositoryProvider);
      final job = await repository.confirmComplete(_jobId);
      state = AsyncData(current.copyWith(job: job, isSubmitting: false));
      ref.invalidate(posterJobsControllerProvider);
      return null;
    } on DioException catch (error) {
